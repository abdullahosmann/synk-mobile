...
4. Next task: **ShareCardRenderer + `share-templates/`, then Phase 3 polish.** Phases 2.6–2.9 are complete, and **all standalone screens from the map are now ported** (`RoutineBuilder`, `ImportRoutine` + `/r/[encoded]`, `PlanDetails`, `WeekEditor`, `Paywall`, `AdaptiveInsights`, `AdaptationHistory`). **ShareCardRenderer + `share-templates/` are now ported** (`src/components/ShareCardRenderer.tsx` — pr/streak/perfect_week; `src/components/share-templates/WorkoutShareTemplates.tsx` — Template1–5 + ProgressComparison), all verified rendering + capturing at 1080×1920 via react-native-view-shot. **Remaining: wire them into the share flows** — WorkoutComplete (`app/workout/complete.tsx`) still captures a simplified 320×568 card; HistoryWorkoutDetail (`app/history/workout/[id].tsx`) and ProgressPhotos (`app/photos.tsx`) likewise approximate. They need a template-picker sheet that renders the chosen ShareCard/Template off-screen, captures via `captureRef`, and shares via expo-sharing (see the web share modals in `../src/screens/main/WorkoutComplete.tsx` / `HistoryWorkoutDetail.tsx` / `ProgressPhotos.tsx`). Then Phase 3 polish. (The app-wide function-style-Pressable sweep is **done** — 0 remain; see the css-interop gotcha below to avoid reintroducing them.)
