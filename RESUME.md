...
> Continue the SYNK web→React Native migration. Read `mobile/RESUME.md` and `MIGRATION_MAP.md` first. The RN app is in `mobile/` on branch `dev` (clean, building). The web app in `src/` is the source of truth — don't modify it. **All routes/screens are ported and the migration is feature-complete vs. web** (Nutrition builders, share-card system, PreSession save/share/exercise-menu all done; remaining "coming soon" strings are 1:1 parity stubs the web also stubs). What's left is the **documented fidelity tail** in the "Fidelity audit" section under Remaining work — pick the next item: PreSession drag-reorder + coach adapt sheet, the Dashboard off-by-default rich cards, or a PlanSettings/Profile spot-check. The only non-parity blocker is the **Android build** (no Android SDK/emulator in this env). Follow the established loop (read web source → port → `npx tsc --noEmit` → screenshot on the sim via the commands below → commit, one logical change per commit, 1:1 faithful). Note: the sim's saved language may be Arabic from an RTL test — toggle in Settings for English.
