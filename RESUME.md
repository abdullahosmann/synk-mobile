...
4. Next task: **port `PreSessionExerciseDetail`** (`../src/screens/main/PreSessionExerciseDetail.tsx` → `mobile/app/workout/exercise/[id].tsx`) — last screen of Phase 2.3. Then Phase 2.4 (Nutrition) and 2.5 (settings/social/history/tracking).

## Status (what's done)

Branch `dev`, 17 feature commits. All screens below are typecheck-clean and verified rendering on the iPhone 17 Pro Max simulator.
...
- **Phase 2.3:** PreSessionExerciseDetail (last one).
...
> Continue the SYNK web→React Native migration. Read `mobile/RESUME.md` and `MIGRATION_MAP.md` first. The RN app is in `mobile/` on branch `dev` (clean, building). The web app in `src/` is the source of truth — don't modify it. Port the next screen: `src/screens/main/PreSessionExerciseDetail.tsx` → `mobile/app/workout/exercise/[id].tsx`, following the established loop (read → port → tsc → screenshot natively → commit). Keep the design 1:1 faithful.
