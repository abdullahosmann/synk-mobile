...
4. Next task: **Phase 2.5 — continue the settings cluster.** Done: `Settings` hub, `AIPermissions`, `PlanSettings`, `EditProfile`, `Subscription`. Port the rest of `/settings/*` in route order: `BlockedAccounts` (`app/settings/blocked.tsx`, 134 lines), `DeleteAccount` (`app/settings/delete-account.tsx`, uses `window.location.href='/'` → `router.replace('/')`), `LegalScreen` (`app/settings/legal/[type].tsx` — one file for terms + privacy). Also `ExcludedExercises` (`app/plan-settings/excluded-exercises.tsx` — linked from PlanSettings, currently Unmatched). After settings: social cluster, CoachChat, history, tracking.

## Status (what's done)

Branch `dev`, 26 feature commits. **Phase 2.3 + 2.4 complete; Phase 2.5 settings cluster in progress (Settings hub + AIPermissions + PlanSettings + EditProfile + Subscription done).** All screens below are typecheck-clean and verified rendering on the iPhone 17 Pro Max simulator.
...
- **Phase 2.5 — settings cluster (in progress):** `app/settings/index.tsx` (Settings hub), `app/settings/ai-permissions.tsx` (AIPermissions), `app/settings/plan.tsx` (PlanSettings — 17 sheets + pendingChange), `app/settings/profile-edit.tsx` (EditProfile), `app/settings/subscription.tsx` (Subscription — premium + free/upgrade views, expo-linear-gradient). `app/settings/_layout.tsx` is a slide stack for the cluster. Note: social brand glyphs are absent in lucide-react-native v1 — substituted Camera/Users/Play.
...
> Continue the SYNK web→React Native migration. Read `mobile/RESUME.md` and `MIGRATION_MAP.md` first. The RN app is in `mobile/` on branch `dev` (clean, building). The web app in `src/` is the source of truth — don't modify it. Continue the settings cluster by porting `src/screens/main/BlockedAccounts.tsx` → `mobile/app/settings/blocked.tsx`, following the established loop (read → port → tsc → screenshot natively → commit). Keep the design 1:1 faithful.
