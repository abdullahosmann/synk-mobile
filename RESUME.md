...
4. Next task: **Phase 2.5 — start the settings cluster** with `Settings` (`../src/screens/main/Settings.tsx` → `mobile/app/settings/index.tsx`), then the rest of `/settings/*` (ai-permissions, plan, profile-edit, subscription, blocked, delete-account, legal). Phase 2.4 is functionally done (Nutrition core, VoiceLog, FABRadialMenu); only the deferred Nutrition sheets remain as an optional follow-up. After settings: social cluster, CoachChat, history, tracking. **Note:** `Settings` uses `localStorage.clear()` → `AsyncStorage.clear()` (preserve language + theme) and `window.location.href='/'` → `router.replace('/')` — see MIGRATION_MAP §6.

## Status (what's done)

Branch `dev`, 21 feature commits. **Phase 2.3 (workout flow) complete; Phase 2.4 done (Nutrition core + VoiceLog + FABRadialMenu).** All screens below are typecheck-clean and verified rendering on the iPhone 17 Pro Max simulator.
...
> Continue the SYNK web→React Native migration. Read `mobile/RESUME.md` and `MIGRATION_MAP.md` first. The RN app is in `mobile/` on branch `dev` (clean, building). The web app in `src/` is the source of truth — don't modify it. Phase 2.4 is done; start Phase 2.5 by porting `src/screens/main/Settings.tsx` → `mobile/app/settings/index.tsx`, following the established loop (read → port → tsc → screenshot natively → commit). Keep the design 1:1 faithful.
