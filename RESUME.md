...
4. Next task: **Phase 2.5 — continue the settings cluster.** `Settings` (the hub) is done. Port the rest of `/settings/*` in route order: `AIPermissions` (`app/settings/ai-permissions.tsx`), `PlanSettings` (`app/settings/plan.tsx`), `EditProfile` (`app/settings/profile-edit.tsx`), `Subscription` (`app/settings/subscription.tsx`), `BlockedAccounts` (`app/settings/blocked.tsx`), `DeleteAccount` (`app/settings/delete-account.tsx`, uses `window.location.href='/'` → `router.replace('/')`), `LegalScreen` (`app/settings/legal/[type].tsx` — one file for terms + privacy). Also `ExcludedExercises` (`app/plan-settings/excluded-exercises.tsx`). After settings: social cluster, CoachChat, history, tracking.

## Status (what's done)

Branch `dev`, 22 feature commits. **Phase 2.3 + 2.4 complete; Phase 2.5 settings cluster started (Settings hub done).** All screens below are typecheck-clean and verified rendering on the iPhone 17 Pro Max simulator.
...
- **Phase 2.5 — settings cluster (in progress):** `app/settings/index.tsx` (Settings hub — grouped list, integrations, time-picker/country/water/sign-out sheets). `app/settings/_layout.tsx` is a slide stack for the cluster. Note: social brand glyphs are absent in lucide-react-native v1 — substituted Camera/Users/Play.
...
> Continue the SYNK web→React Native migration. Read `mobile/RESUME.md` and `MIGRATION_MAP.md` first. The RN app is in `mobile/` on branch `dev` (clean, building). The web app in `src/` is the source of truth — don't modify it. The Settings hub is done; continue the settings cluster by porting `src/screens/main/AIPermissions.tsx` → `mobile/app/settings/ai-permissions.tsx`, following the established loop (read → port → tsc → screenshot natively → commit). Keep the design 1:1 faithful.
