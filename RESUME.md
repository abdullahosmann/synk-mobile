...
4. Next task: **Phase 2.5 — continue the settings cluster.** Done: `Settings` hub, `AIPermissions`, `PlanSettings`. Port the rest of `/settings/*` in route order: `EditProfile` (`app/settings/profile-edit.tsx`, 256 lines), `Subscription` (`app/settings/subscription.tsx`), `BlockedAccounts` (`app/settings/blocked.tsx`), `DeleteAccount` (`app/settings/delete-account.tsx`, uses `window.location.href='/'` → `router.replace('/')`), `LegalScreen` (`app/settings/legal/[type].tsx` — one file for terms + privacy). Also `ExcludedExercises` (`app/plan-settings/excluded-exercises.tsx` — linked from PlanSettings, currently Unmatched). After settings: social cluster, CoachChat, history, tracking.

## Status (what's done)

Branch `dev`, 24 feature commits. **Phase 2.3 + 2.4 complete; Phase 2.5 settings cluster in progress (Settings hub + AIPermissions + PlanSettings done).** All screens below are typecheck-clean and verified rendering on the iPhone 17 Pro Max simulator.
...
> Continue the SYNK web→React Native migration. Read `mobile/RESUME.md` and `MIGRATION_MAP.md` first. The RN app is in `mobile/` on branch `dev` (clean, building). The web app in `src/` is the source of truth — don't modify it. Continue the settings cluster by porting `src/screens/main/EditProfile.tsx` → `mobile/app/settings/profile-edit.tsx`, following the established loop (read → port → tsc → screenshot natively → commit). Keep the design 1:1 faithful.
