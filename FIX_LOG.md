...
| **M1** no list virtualization | `[FIXED+VERIFIED]` | `d3bf592`, `fee309d` | `M1_community_flatlist.png`, `M1_history_sectionlist.png`, `M1_food_search_flatlist.png` | Virtualized the three genuinely-unbounded lists: community feed → `FlatList` (drops the N×40ms `FadeInDown` stagger); workout history → `SectionList` (months=sections, card extracted; calendar/empty stay in a plain ScrollView so nothing nests a VirtualizedList); Nutrition food search (`displayFoods` over all foods) → `FlatList` (label row = `ListHeaderComponent`, "no results" = `ListEmptyComponent`). All sim-verified. Per-exercise set history is already capped at `sessions.slice(0,10)` and Nutrition per-day meal sections are a handful — both bounded, intentionally left as ScrollView (converting would nest VirtualizedLists for no gain). |

## Phase 8 — Dynamic Type

| Item | Status | Commit | Screenshot | Notes |
|------|--------|--------|------------|-------|
| **P5** dynamic-type overflow risk; no scaling cap | `[FIXED+CODE-ONLY]` | `5266607` | — | Added per-variant `maxFontSizeMultiplier` in `AppText` (display/stat 1.3, body/caption 1.5–1.6), caller-overridable, so large OS font sizes don't overflow the many fixed-height pills/rows/stat lines. The earlier inline-fontSize clip fix + this cap together cover line-cropping and container overflow. (Min-heights on every fixed-height pill is a further follow-up.) |

### Phase 9 — B4 long-tail (close buttons + calendar) — ✅
`[FIXED+CODE-ONLY]` `7d1d30e`, `6538f16`. Labeled the remaining interactive icon-only controls: standalone modal/overlay close (X) buttons (morning-checkin, dashboard welcome, muscle-recovery cancel, workout active end/settings, RoutineBuilder library, Nutrition portion/quick-add/dismiss-suggestion/custom-meal-remove, RoutineReplacementSheet) + history calendar day cells (date + workout count). **a11y label coverage 11 → 65.** B4 is now substantially complete — all standard buttons, nav, sheets, tabs, send/mic, close/X, FAB actions, list rows, and calendar cells are labeled. Verified by inspection + a post-sweep boot sanity-check (dashboard renders clean).

**Remaining (not started), per UIUX_AUDIT.md:** F8 (deferred), P5 follow-up (fixed `height:` → `minHeight:` on text pills — broad/risky layout sweep, low value vs. the maxFontSizeMultiplier cap already in place), P4 (dead voice-log states — harmless until a real recognizer). Android build still blocked (no toolchain).
